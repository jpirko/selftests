...
rtr_create()
{
	...
	ip link add dev vrf-rtr type vrf table 30
	ip -4 route add vrf vrf-rtr unreachable default
	ip -6 route add vrf vrf-rtr unreachable default
	ip link set dev $rtr1 master vrf-rtr
	ip link set dev $rtr2 master vrf-rtr
	ip link set dev vrf-rtr up
...
rtr_destroy()
{
	...
	ip link set dev vrf-rtr down
	ip link set dev $rtr2 nomaster
	ip link set dev $rtr1 nomaster
	ip -6 route del vrf vrf-rtr unreachable default
	ip -4 route del vrf vrf-rtr unreachable default
	ip link del dev vrf-rtr
...
ping_test()
{
	...
	err=$?
	if [[  $err -eq 0 && $should_fail == "false" || \
	       $err -ne 0 && $should_fail == "true" ]]; then
		echo "PASS: ping"
	else
		echo "FAIL: ping"
		ret=1
...
ip route del vrf vrf-rtr 192.168.101.0/24
ip route del vrf vrf-rtr dead::/64
